...
import { ResponseStatus, SurveyResponseData } from '../types';

export interface QuestionAnswerPayload { questionId: string; rating: number; comment?: string; }

export interface MySurveyResponseStatus {
  surveyId: string;
  responseId: string;
...
export const getMySurveyResponseStatuses = async (): Promise<MySurveyResponseStatus[]> =>
  (await api.get('/responses/mine')).data;
export const startResponse = async (surveyId: string): Promise<SurveyResponseData> =>
  (await api.post(`/surveys/${surveyId}/responses`)).data;
